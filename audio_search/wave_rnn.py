...
model_ = QueryEncoder().cuda()
model_.train()
optimiser = optim.Adam(model_.parameters(), lr=0.0001)
...
class poolingmodel(nn.Module):
    ...
    def __init__(self, dim):
        super().__init__()
        self.pool = nn.MaxPool1d(dim)

    def forward(self, x):
        return self.pool(x)

pooling_model = poolingmodel(128).cuda()

if args.generate:
    model.do_generate(paths, step, optimiser, dataset, test_index, model_, pooling_model, use_half=use_half,
    ...
    model.do_train(paths, dataset, optimiser, epochs=1000, batch_size=16, step=step, lr=1e-4, use_half=use_half,
